...
| 2 | Save to `data/chinese/chinese_icu_data.csv` | ⬜ |
| 3 | Ensure SOFA scores computed (or compute them) | ⬜ |
| 4 | Document column names (outcome, year, gender, age, etc.) | ⬜ |
| 5 | Update `code/config.py` → `chinese_icu` entry with column mappings | ⬜ |
| 6 | Run `python code/01_explore_data.py` (set `ACTIVE_DATASET='chinese_icu'`) | ⬜ |
| 7 | Run `python code/02_drift_analysis.py` | ⬜ |
| 8 | Share results on WhatsApp | ⬜ |

## Placeholder Scripts

Placeholder scripts are ready in `code/chinese/`:
- `code/chinese/01_explore_data.py` - Copy from `code/mimic/` and adapt
- `code/chinese/02_drift_analysis.py` - Copy from `code/mimic/` and adapt
