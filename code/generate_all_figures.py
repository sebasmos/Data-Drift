...
def create_fig1_gender_race(results=None):
    ...
    *Saltz/Zhejiang have no race data — show Male vs Female only.
    ...
    if not panel_data:
        print("  Skipped fig2_gender_race: no data available")
        return
    ...
    plt.tight_layout()
    fig.savefig(FIGURES_DIR / 'fig2_gender_race.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Saved: fig2_gender_race.png")
...
def create_fig2_age_race(results=None):
    ...
    if not panel_data:
        print("  Skipped fig3_age_race: no data available")
        return
    ...
    plt.tight_layout()
    fig.savefig(FIGURES_DIR / 'fig3_age_race.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Saved: fig3_age_race.png")
...
def create_fig3_gender_age_race(results=None):
    ...
    if not panel_data:
        print("  Skipped fig4_gender_age_race: no data available")
        return
    ...
    plt.tight_layout()
    fig.savefig(FIGURES_DIR / 'fig4_gender_age_race.png', dpi=300, bbox_inches='tight')
    plt.close()
    print("Saved: fig4_gender_age_race.png")
...
def main():
    ...
    # ============================================================
    print("\n" + "-" * 50)
    print("MAIN FIGURES (6)")
    print("-" * 50)

    # Fig 1: Study flow — fig1_study_flow.svg already exists (manually created)
    print("Fig 1: study flow (fig1_study_flow.svg exists, skipping)")

    # Fig 2: Gender-Race SOFA bar chart
    print("Generating Fig 2 (Gender-Race SOFA)...")
    create_fig1_gender_race(results)

    # Fig 3: Age-Race SOFA bar chart
    print("Generating Fig 3 (Age-Race SOFA)...")
    create_fig2_age_race(results)

    # Fig 4: Gender-Age-Race SOFA bar chart
    print("Generating Fig 4 (Gender-Age-Race SOFA)...")
    create_fig3_gender_age_race(results)

    # Fig 5-6: Nursing care phenotypes (copied from supplementary after they're generated)
    # These are generated by supplementary_analysis.py — we copy them to main figures dir
    src5 = SUPPLEMENTARY_FIGURES_DIR / 'figS1_mimic_mouthcare.png'
    src6 = SUPPLEMENTARY_FIGURES_DIR / 'figS2_mimic_mechvent.png'
    dst5 = FIGURES_DIR / 'fig5_mouthcare.png'
    dst6 = FIGURES_DIR / 'fig6_mechvent.png'
    import shutil
    if src5.exists():
        shutil.copy2(src5, dst5)
        print(f"Fig 5: Mouthcare -> {dst5.name}")
    else:
        print(f"Fig 5: Mouthcare source not found ({src5}), will be generated by supplementary_analysis.py")
    if src6.exists():
        shutil.copy2(src6, dst6)
        print(f"Fig 6: Mechanical ventilation -> {dst6.name}")
    else:
        print(f"Fig 6: Mechanical ventilation source not found ({src6}), will be generated by supplementary_analysis.py")

    # ============================================================
    # SUPPLEMENTARY: single-subgroup per-dataset
    # ============================================================
    print("\n" + "-" * 50)
    ...
    for dataset_key, dataset_name in dataset_names.items():
        print(f"\nGenerating supplementary for {dataset_key}...")
