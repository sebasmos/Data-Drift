CREATE temp TABLE datasheet as
with htwt as
(
    ...
    from `lcp-consortium.eicu_crd_ii_v0_1_0.patient` as pt
)

select
  -- patient identifiers
  pt.patientunitstayid
  , 'eicu_' || cast(pt.patientunitstayid as STRING) as encounter_id
  ...
  , cast(null as STRING) as hospital_type
...
--   , lab_h1.inr_min as h1_inr_min -- TODO: let jesse know that both inr were d1 before
...
--   , case when apv.oobventday1 = -1 then NULL else apv.oobventday1 end as ventilated_apache
  ...
  , cast(apr.predictedicumortality as FLOAT64) as apache_4a_icu_death_prob
  , cast(apr.predictedhospitalmortality as  FLOAT64) as apache_4a_hospital_death_prob
